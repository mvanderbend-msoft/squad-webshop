...
// GET /api/favorites — list authenticated user's favorited products
router.get('/', (req, res) => {
  const rows = db
    .prepare(
...
router.post('/:productId', (req, res) => {
  ...
  // Idempotent insert — second call is a no-op, not an error
  db.prepare(
...
router.delete('/:productId', (req, res) => {
  ...
  // Idempotent delete — removing a non-existent favorite still returns 200
